...
export default function NewTestCasePage() {
  ...
  useEffect(() => {
    ...
    if (activeProject && activeProject !== "ALL") {
      setForm(f => ({ ...f, project: activeProject }));
    } else {
      setForm(f => ({ ...f, project: "" }));
    }
  }, [activeProject]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    // Use pruned steps for the payload
    const res = await fetch("/api/testcases", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, steps: prunedSteps, project: activeProject, createdBy: currentUser })
    });
        ...
        <header className="page-header">
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>Fill in all required fields to create a test case</p>
            </div>
          </div>
                ...
                <div style={{ gridColumn: "span 3" }}>
                  ...
                  <select 
                    ...
                    onChange={e => {
                      ...
                      if (!v) return;
                      if (!(form as any).versionNumbers.includes(v)) {
                         setForm(f => ({ ...f, versionNumbers: [...(f as any).versionNumbers, v] }));
                      }
                    }} 
                    style={selectStyle}>
